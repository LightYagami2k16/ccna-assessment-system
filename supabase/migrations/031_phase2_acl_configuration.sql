...
-- PHASE 2D: STANDARD AND EXTENDED ACL CONFIGURATION AND GRADING
-- Requires migrations 020 through 030.
-- =========================================================
...
create or replace function public.canonical_cli_acl_id(
  p_value text
)
returns text
language sql
...
as $$
  select case
    when trim(coalesce(p_value, '')) ~ '^[0-9]+$'
      then trim(p_value)
  ...
  select coalesce(
    case p_criterion->>'type'
      ...
      when 'acl_exists' then
        coalesce(p_state->'accessLists', '{}'::jsonb)
      ...
      when 'acl_entry' then
        exists (
          ...
          where lower(
            regexp_replace(
              trim(acl_entry.value->>'statement'),
              '\s+',
              ' ',
              'g'
            )
...
revoke all on function public.canonical_cli_acl_id(text) from public;
