-- Phase 3: grade successful end-to-end SSH sessions per initiating device.
          ...
          from jsonb_array_elements_text(
            coalesce(
              public.cli_criterion_device_state(
                p_state,
                p_criterion
              )->'successfulSshConnections',
              '[]'::jsonb
            )
          ) successful_ssh(value)
          where trim(successful_ssh.value) =
...
begin
  ...
  if v_definition is null then
    raise exception
      'cli_criterion_is_met(jsonb, jsonb) must exist before migration 056';
  end if;

  if position('when ''connectivity_ssh'' then' in v_definition) = 0 then
...
is 'Grades device-aware configuration and end-to-end topology verification criteria.';
